...
# Define the precise mail templates
mail_templates = {
    "monday": """БОЕВОЙ ПРИКАЗ: ПОНЕДЕЛЬНИК ДЕНЬ 1
Улучшение Ворона Активность

Главные задачи: Выполняем Задания Сокола и улучшаем Ворона эссенцией Ворона ради максимума очков. Проводим основные атаки митингов.
...
# The user also says "Убери из шапок слово «Dream», удали лишние скобки"
# Let's clean shortChatTemplate as well in the file by replacing "[Dream] " with ""
# Let's also remove brackets in phase names and day names if they were added. Wait, the prompt gave exact text for mail templates.

for day_id, template in mail_templates.items():
    # Find the object for this day
    # We will use regex to find id: "monday", ... and insert exactMailTemplate
    
    # regex pattern to find the object and add the property
    pattern = r'(id:\s*"' + day_id + r'".*?shortChatTemplate:\s*"[^"]*")'
    replacement = r'\1,\n    exactMailTemplate: `' + template + r'`'
    content = re.sub(pattern, replacement, content, flags=re.DOTALL)

# Let's also remove "[Dream] " from shortChatTemplate
content = re.sub(r'shortChatTemplate:\s*"\[Dream\]\s*', 'shortChatTemplate: "', content)
